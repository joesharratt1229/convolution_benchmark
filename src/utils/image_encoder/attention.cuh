...
__device__ inline accT calculate_block_max(accT warp_max, int tid, int seq_len) {
    __shared__ accT warpMaxes[NUM_WARPS];
...
template <typename accT, int warps_per_sequence>
__device__ inline accT calculate_block_sum(accT warp_sum, int tx, int ty, int seq_len) {
    
    __shared__ accT warpSums[blockDim.y * warps_per_sequence];
    int lane_id = tx % WARP_SIZE;
    int warp_id = tx / WARP_SIZE;

    if (lane_id == 0) {
        warpSums[warps_per_sequence * ty + warp_id] = warp_sum;
    }
    ...
    if (warp_id == 0) {
        int num_warps = (blockDim.x + WARP_SIZE - 1) / WARP_SIZE;
        accT thread_val = (lane_id < num_warps) ? warpSums[warps_per_sequence * ty + lane_id] : 0;
        block_sum = tree_reduction_sum(thread_val);

        if (lane_id == 0) {
            warpSums[warps_per_sequence * ty] = block_sum;
        }
    }

    __syncthreads();
    
    return warpSums[warps_per_sequence * ty];
}


template <typename T, typename accT, int embed_dim, int seq_len, int query_elements_per_block, int warps_per_sequence>
__global__ void flash_attention_kernel(const T* query, const T* key, const T* value, T* output, accT scale) {
    int seq_id = threadIdx.y % seq_len;
    int query_seq_idx = (blockIdx.x * TC * query_elements_per_block) + (TC * seq_id);
...
void flash_attention_kernel_wrapper(const T* query, const T* key, const T* value, T* output, int num_heads) {
    ...
    const int Bc = seq_len/(query_elements_per_block * TC);

    dim3 block_size(seq_len, query_elements_per_block);
    dim3 grid_size(Bc, num_heads);

    flash_attention_kernel<T, accT, embed_dim, seq_len, query_elements_per_block, warps_per_sequence><<<grid_size, block_size>>>(d_query, d_key, d_value, d_output, scale);
