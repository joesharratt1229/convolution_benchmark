...
__device__ inline accT calculate_block_max_row(accT val, int tid_x, int tid_y) {
    ...
    extern __shared__ accT warpMaxes[];  
...
__device__ inline accT calculate_block_sum_row(accT val, int tid_x, int tid_y) {
    ...
    extern __shared__ accT warpSums[];  
...
__global__ void flash_attention_kernel(const T* query, const T* key, const T* value, T* output, accT scale) {
    ...
    for (int q = 0; q < Tc; q++) {
        ...
        accT block_max = calculate_block_max_row(warp_max, threadIdx.x, threadIdx.y);
        ...
        accT rescaled_sum_block = calculate_block_sum_row(rescaled_sum, threadIdx.x, threadIdx.y);
        ...
        for (int d = 0; d < embed_dim; d ++) {
            ...
            accT block_sum = calculate_block_sum_row(warp_sum, threadIdx.x, threadIdx.y);
